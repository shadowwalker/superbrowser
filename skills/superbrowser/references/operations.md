...
| browser_unavailable | 69 | Follow setup to start or repair the shared browser connection |
...
Use an absolute runtime and entrypoint, preserve the user's HOME, and pass any CDP override and required environment variables explicitly. Cron often has a minimal PATH and does not inherit the interactive shell environment. The browser must run in the same logged-in macOS session; a sleeping or logged-out Mac cannot reliably run this local GUI workflow. Keep browser startup separate from workflow code.
...
Use the selected agent's documented headless invocation and existing credentials. Do not assume the coding agent's model credentials configure Stagehand model calls. Trigger one repair attempt per failed run, only after the workflow releases its connection, and prevent recursive repair loops. Authentication expiry goes to the user; a missing browser goes through setup. Persistent or ambiguous failures need human review.
